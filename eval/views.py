...
def obtener_venta_abierta():
    venta_abierta = Venta.objects.filter(abierta=True, operador=3).first()


    if not venta_abierta:
        venta_abierta = Venta.objects.create(abierta=True, operador=3)
...
def finalizar_venta(request):
    venta_actual = get_object_or_404(Venta, abierta=True, operador=3)
